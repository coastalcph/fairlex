...
then
    ALGO=deepCORAL
    PARAM="--groupby_fields respondent"
fi
nvidia-smi
...
--log_dir logs/scotus_v03/$ATTRIBUTE/$ALGO/official --split_scheme official \
...
COMMAND="run_expt.py --dataset scotus --algorithm $ALGO $PARAM --root_dir data \
--log_dir logs/scotus_v03/$respondent/$ALGO/temporal --split_scheme temporal \
--dataset_kwargs protected_attribute=$ATTRIBUTE \
--save_best --seed $RAND_NUM"
echo ''
...
COMMAND="run_expt.py --dataset scotus --algorithm $ALGO $PARAM --root_dir data \
--log_dir logs/scotus_v03/$respondent/$ALGO/uniform --split_scheme uniform \
--dataset_kwargs protected_attribute=$ATTRIBUTE \
--save_best --seed $RAND_NUM"
echo ''
