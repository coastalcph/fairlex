...
CUDA_VISIBLE_DEVICES=${GPU_NUMBER} python run_expt.py --use_wandb True --dataset ${DATASET} --algorithm ${ALGORITHM} --batch_size ${BATCH_SIZE} --root_dir data/datasets --log_dir "./logs/${DATASET}/${ALGORITHM}/${GROUP_FIELD}/seed_1" --fp16 True --seed 1 --groupby_fields ${GROUP_FIELD} --n_groups_per_batch ${N_GROUPS}
